...
def route_api_songs():
    cursor = db.songs.find({'enabled': True}, {'_id': False, 'enabled': False}).sort("order", DESCENDING)
    songs = list(cursor)
...
def send_src(ref):
    res = send_from_directory("public/src", ref)
    cache_wrap(res, 3600)
    return res

@app.route(basedir + "assets/<path:ref>")
def send_assets(ref):
    res = send_from_directory("public/assets", ref)
    cache_wrap(res, 3600)
    return res

@app.route(basedir + "songs/<path:ref>")
def send_songs(ref):
    res = send_from_directory("public/songs", ref)
    cache_wrap(res, 604800)
    return res

@app.route(basedir + "manifest.json")
def send_manifest():
    res = send_from_directory("public", "manifest.json")
    cache_wrap(res, 3600)
    return res

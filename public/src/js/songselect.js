...
"ranking": {
    sort: 0,
    background: "#b3e5fc",
    border: ["#81d4fa", "#0288d1"],
    outline: "#4fc3f7"
},

"sourceCode": {
    sort: 0,
    background: "#d7ccc8",        // 明るめのブラウンベース（背景）
    border: ["#8d6e63", "#5d4037"], // 中〜濃いブラウンのグラデーション
    outline: "#6d4c41",           // 濃いブラウンでアクセント
},
...
"easySettings": {
    "sort": 0,
    "background": "#d1c4e9",  // 紫色の背景
    "border": ["#b39ddb", "#9575cd"],  // 紫色の境界線
    "outline": "#673ab7"  // 紫色のアウトライン
},
			...
			"default": {
				...
				border: [`rgba(255, 255, 255, 0.3)`, "%random%"],
				outline: `#333`,
				infoFill: `#333`
			}
		...
		const titleSort = easySettings.get("titleSort") === 1;
		if (titleSort) {
...
// カスタム設定を曲アイテムとして追加
const siSkin = this.songSkin.easySettings;
const sis = easySettings.asSongItems(siSkin);
for (const si of sis) {
  this.songs.push(si);
}
		...
		this.diffOptions = [{
			...
			text: strings.scoreRanking,
			fill: "#6ec1e5",
			iconName: "score-ranking",
			iconFill: "#9fd9f0",
			letterSpacing: 4
		}]
		...
		this.songSelecting = {
			speed: easySettings.get("songSelectingSpeed"),
			resize: 0.3,
			scrollDelay: 0.1
		}
		this.wheelScrolls = 0
		...
		if (this.touchSearchBtn) {
			...
			pageEvents.add(this.touchSearchBtn, "mouseup", openSearch)
		}
	...
	diffSelMouse(x, y){
		if(this.state.locked === 0){
			if(0 < x && x < 223 - 72 * (this.diffOptions.length - 4) + 72 * this.diffOptions.length && 132 < y && y < 436){
				return Math.floor((x - 223 + 72 * (this.diffOptions.length - 4)) / 72)
		...
		if(p2.session && !fromP2 && (!currentSong.action || !currentSong.p2Enabled)){
			...
			if(currentSong.courses){
				...
				this.toPlugins()
                        ...
                        // カスタムメニューの実行処理
else if (currentSong.action === "ranking") {
this.playSound("se_don");
...
window.location.href = "/ranking/";
                            ...
                            setTimeout(() => {
// ソースコードのリンク
window.location.href = "https://forgejo.win/yuuki/taiko-web-ide";
                            }, 500);
...
window.location.href = "https://litey.trade/";
						...
						} else if (easySettings.keys().includes(currentSong.action)) {
							this.playSound("se_don");
// 簡単設定を起動
setTimeout(() => { easySettings.launchEditor(currentSong.action); }, 100);
						}
		}
		this.pointer(false)
	}
			...
			innerContent: (x, y, w, h) => {
				...
				if(screen === "title" || screen === "titleFadeIn" || screen === "song"){
					...
					for(var i = 0; i < this.diffOptions.length; i++){
						var _x = x + 62 - (this.diffOptions.length - 4) * 72 + i * 72
						var _y = y + 67
...
// 日付の描画
if (order) {
const created = new Date(order / 1e6).toLocaleString();
...
// 曲の長さの描画
if (preview && preview?.sources?.size > 0) {
const duration = preview?.buffer?.duration ?? 0;
const date = new Date(duration * 1e3);
const time = date.toISOString().split("T")[1].split("Z")[0].split(".")[0].replace(/^00:/, "");
...
// 再生回数
if (song.viewsUpdate) {
  song.viewsUpdate = false;
  song.viewsDate = ms;

  fetch(`/api/views/${encodeURIComponent(song.id)}`)
    .then((res) => res.json())
    .then((obj) => {
      song.views = obj?.count ?? null;
    });
}

if (song.id !== undefined && song.views === undefined) {
  song.views = null;
  song.viewsUpdate = true;
}
if (ms - song.viewsDate >= 60 * 1000) {
  song.viewsUpdate = true;
}

if (song.views || song.views === 0) {
this.draw.layeredText({
  ctx,
  text: `再生回数: ${song.views}`,
  fontSize: 30,
  fontFamily: this.font,
  x: 30,
  y: 30,
...
// プレイ回数
if (song.playedUpdate) {
  song.playedUpdate = false;
  song.playedDate = ms;

  fetch(`/api/played/v2/hash/${encodeURIComponent(song.title)}`)
    .then((res) => res.json())
    .then((obj) => {
      song.played = obj?.count ?? null;
    });
}

if (song.id !== undefined && song.played === undefined) {
  song.played = null;
  song.playedUpdate = true;
}
if (ms - song.playedDate >= 60 * 1000) {
  song.playedUpdate = true;
}

if (song.played || song.played === 0) {
this.draw.layeredText({
  ctx,
  text: `プレイ回数: ${song.played}`,
  fontSize: 30,
  fontFamily: this.font,
  x: 30,
  y: 30 + 50,
...
for (const song of this.songs) {
  if (easySettings.keys().includes(song.action)) {
    const changed = easySettings.isEdited(song.action);

    if (changed && !song.title.startsWith(emoji)) {
      song.title = `${emoji}${song.title}`;
    } else if (!changed && song.title.startsWith(emoji)) {
...
// 再生回数を追加
const {id} = this.songs[this.selectedSong];
if (id) {
  fetch(`/api/views/inc/${id}`, {method: "POST"});
}
		...
		const playbackRate = easySettings.get("playbackRate");
		this.preview.playLoop(delay / 1000, false, prvTime, undefined, undefined, playbackRate)
	}
	endPreview(force){
		this.previewId++
	...
	addSong(song){
		...
		addedSong.skin = JSON.parse(JSON.stringify(addedSong.skin));
		if (addedSong.skin.border[1] === "%random%") {
			const gen = () => Math.floor(Math.random() * 256);
			addedSong.skin.border[1] = `rgba(${gen()}, ${gen()}, ${gen()}, 0.7)`;
		}
...
window.location.href = `/delete/?id=${encodeURIComponent(id)}`;
	...
	toScoreRanking() {
		const {title} = this.songs[this.selectedSong];
window.location.href = `/score-ranking/?hash=${encodeURIComponent(title)}`;
	}
}

...
const progressBar = easySettings.get("progressBar") === 1;

if(!this.multiplayer && progressBar) {

// 描画
const circles = this.controller.game.songData.circles;

const played = circles.filter((c) => c.isPlayed).length;
const total = circles.length;

const barY = 0;
const barH = 100;
const playedW = played * winW / total;
const nowW = this.circlesNow * winW / total;

ctx.globalAlpha = 0.7;
ctx.fillStyle = "#" + easySettings.get("colorOfProgressBar").toString(16).padStart(6, "0");
ctx.fillRect(0, barY, playedW, barH);
ctx.fillStyle = "#eee";
ctx.fillRect(playedW, barY, nowW, barH);
ctx.globalAlpha = 1;

}

const visualizer = easySettings.get("visualizer") === 1;

if (!this.multiplayer && visualizer) {

const sound = this.controller.game.mainAsset;

// ビジュアライザーの描画

const freqData = sound.gain.getFrequencyData();

const k = easySettings.get("heightOfVisualizer");
const peakVol = sound.getPeakVolume();

const freeH = winH - (this.portrait ? 472 : 360);
const resizeY = 1 - Math.exp(-k * peakVol);
const maxH = freeH * resizeY;

ctx.globalAlpha = 0.7;
for (var i = 0; i < freqData.length; i++) {
  const v = freqData[i];

  const x = i * winW / freqData.length;
  const width = winW / freqData.length;
  const height = v * maxH / (256 - 1);

  ctx.fillStyle = "#" + easySettings.get("colorOfVisualizer").toString(16).padStart(6, "0");
  ctx.fillRect(x, winH - height, width, height);
}
ctx.globalAlpha = 1;
...
// ビジュアライザーの終了
		...
		const circleChaosMode = easySettings.get("circleChaosMode") === 1;
		const circleChaosModeSlotPosXMul = this.multiplayer ? (this.player === 2 ? 0.75 : 0.25) : 0.5;
		...
		if(this.portrait){
			...
			this.slotPos = {
				x: circleChaosMode ? winW * circleChaosModeSlotPosXMul : 66,
				y: circleChaosMode ? winH / 2 : frameTop + 375,
				size: 100,
				paddingLeft: 0
			}
			...
			this.slotPos = {
				x: circleChaosMode ? winW * circleChaosModeSlotPosXMul : 413,
				y: circleChaosMode ? winH / 2 : frameTop + 257,
				size: 106,
				paddingLeft: 332
			}
			this.scorePos = {
		...
		if (!this.multiplayer && settings.getItem("showHs")) {
			...
			const baisoku = easySettings.get("baisoku")
		...
		// show ms
if (!this.multiplayer && settings.getItem("showMs")) {

var pos = this.portrait ? 500 : 400;
if (settings.getItem("showBpm")) {
  pos += 50;
}
if (settings.getItem("showHs")) {
  pos += 50;
}
...
if(!this.multiplayer && settings.getItem("showCirclesNow")) {
...
const text = `ノーツ: ${played} + ${this.circlesNow} / ${total}`;
...
if(!this.multiplayer && settings.getItem("showSoundNow")) {
...
if (settings.getItem("showCirclesNow")) {
  //pos += 50;
}

// 描画
const sound = this.controller.game.mainAsset;
const peakFreq = Math.floor(sound.getPeakFrequency());
const peakVol = Math.floor(sound.getPeakVolume() * 100);
const rmsVol = Math.floor(sound.getRMSVolume() * 100);

const text = `周波数領域: ${peakFreq} Hz (ピーク)`;
const text2 = `時間領域: ${peakVol} (ピーク) | ${rmsVol} (RMS) / 100`;
  ...
  { fill: "#fff" }
	...
	drawMeasures(){
		...
		measures.forEach(measure => {
			var timeForDistance = this.posToMs(distanceForCircle, measure.speed * easySettings.get("baisoku"))
			var startingTime = measure.ms - timeForDistance + this.controller.videoLatency
			var finishTime = measure.ms + this.posToMs(this.slotPos.x - this.slotPos.paddingLeft + 3, measure.speed * easySettings.get("baisoku")) + this.controller.videoLatency
			if(measure.visible && (!measure.branch || measure.branch.active) && ms >= startingTime && ms <= finishTime){
				var measureX = this.slotPos.x + this.msToPos(measure.ms - ms + this.controller.videoLatency, measure.speed * easySettings.get("baisoku"))
	...
	drawCircles(circles){
		...
		for(var i = circles.length; i--;){
			...
			var speed = circle.speed * easySettings.get("baisoku")
	...
	drawCircle(circle, circlePos, fade){
		...
		const doron = easySettings.get("doron") === 1;
		...
		if(!circlePos){
			...
			const distance = this.msToPos(circleMs - ms + this.controller.videoLatency, speed);

			const circleChaosMode = easySettings.get("circleChaosMode") === 1;
			if (circleChaosMode) {
				const angle = ((ms + distance) * (Math.PI / 180)) % (2 * Math.PI);

				circlePos.x += distance * Math.cos(angle);
				circlePos.y += distance * Math.sin(angle);
			} else {
				circlePos.x += distance;
			}
		}
		if(animated){
			var noteFace = {
		...
		if(type === "don" || type === "daiDon" && played === 1){
			...
			if (circle.timesHit > 0 && audioMS < endTime + 2000) {
				...
				var fixedY;
				if (this.player === 2) {
					fixedY = this.slotPos.y + h / 2 + 20;
				} else {
					fixedY = this.slotPos.y - h / 2 - 180;
				}

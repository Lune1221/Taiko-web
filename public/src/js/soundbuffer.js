class SoundBuffer{
	...
	createSource(sound){
		...
		source.connect(sound.gain.analyser);
		return source
	}
	pageClicked(){
...
class SoundGain{
	...
	init(soundBuffer, channel){
		...
		this.analyser = soundBuffer.context.createAnalyser();
	}
	load(url){
...
class Sound{
	...
	playLoop(time, absolute, seek1, seek2, until){
		...
		this.loop = {
			started: time + (until - seek1) / playbackRate,
			playbackRate,
			seek: seek2,
			until: until
		}
	...
	addLoop(){
		while(this.getTime() > this.loop.started - 1){
			...
			this.loop.started += (this.loop.until - this.loop.seek) / this.loop.playbackRate
		}
	}
	play(time, absolute, seek, until){
		...
		this.gain.analyser.fftSize = easySettings.get("fftSize");
	...
	getPeakFrequency() {
		const freqData = this.gain.getFrequencyData();
		...
		const sampleRate = this.gain.analyser.context.sampleRate;
		const peak = max * (sampleRate / 2) / freqData.length;

		return peak;
	}
	getRMSVolume() {
		const timeData = this.gain.getTimeDomainData();
